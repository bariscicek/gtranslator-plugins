...
/* This is a modified version of gedit-automatic-spell-checker.c */
...
struct _GtranslatorAutomaticSpellChecker {
	GtkTextBuffer		*doc;
	GtranslatorTab		*tab;
	GSList 			*views;
...
static void gtranslator_automatic_spell_checker_free_internal (GtranslatorAutomaticSpellChecker *spell);

static void
view_destroy (GtranslatorView *view, GtranslatorAutomaticSpellChecker *spell)
{
	gtranslator_automatic_spell_checker_detach_view (spell, view);
}

static void
check_word (GtranslatorAutomaticSpellChecker *spell, GtkTextIter *start, GtkTextIter *end) 
{
	gchar *word;
	...
	*/
	...
	if (!gtranslator_spell_checker_check_word (spell->spell_checker, word, -1)) 
	{
		/*
...
static void
check_range (GtranslatorAutomaticSpellChecker *spell, 
	     GtkTextIter                 start, 
	     GtkTextIter                 end,
	     gboolean                    force_all) 
{
...
static void
check_deferred_range (GtranslatorAutomaticSpellChecker *spell, 
		      gboolean                    force_all) 
{
...
insert_text_before (GtkTextBuffer *buffer, GtkTextIter *iter,
		gchar *text, gint len, GtranslatorAutomaticSpellChecker *spell) 
{
...
delete_range_after (GtkTextBuffer *buffer, GtkTextIter *start, GtkTextIter *end, 
		GtranslatorAutomaticSpellChecker *spell) 
{
...
static void
remove_tag_to_word (GtranslatorAutomaticSpellChecker *spell, const gchar *word)
{
	GtkTextIter iter;
...
static void
add_to_dictionary (GtkWidget *menuitem, GtranslatorAutomaticSpellChecker *spell) 
{
	gchar *word;
	...
	gtranslator_spell_checker_add_word_to_personal (spell->spell_checker, word, -1);

	g_free (word);
}

static void
ignore_all (GtkWidget *menuitem, GtranslatorAutomaticSpellChecker *spell) 
{
	gchar *word;
	...
	gtranslator_spell_checker_add_word_to_session (spell->spell_checker, word, -1);

	g_free (word);
}

static void
replace_word (GtkWidget *menuitem, GtranslatorAutomaticSpellChecker *spell) 
{
	gchar *oldword;
	const gchar *newword;
	...
	gtranslator_spell_checker_set_correction (spell->spell_checker, 
...
static GtkWidget *
build_suggestion_menu (GtranslatorAutomaticSpellChecker *spell, const gchar *word) 
{
	...
	suggestions = gtranslator_spell_checker_get_suggestions (spell->spell_checker, word, -1);
...
static void
populate_popup (GtkTextView *textview, GtkMenu *menu, GtranslatorAutomaticSpellChecker *spell) 
{
	GtkWidget *img, *mi;
	GtkTextIter start, end;
	char *word;
...
void
gtranslator_automatic_spell_checker_recheck_all (GtranslatorAutomaticSpellChecker *spell)
{
	GtkTextIter start, end;
...
static void 
add_word_signal_cb (GtranslatorSpellChecker          *checker, 
		    const gchar                *word, 
		    gint                        len,
		    GtranslatorAutomaticSpellChecker *spell)
{
...
static void 
set_language_cb (GtranslatorSpellChecker               *checker,
		 GtranslatorAutomaticSpellChecker      *spell)
{
	gtranslator_automatic_spell_checker_recheck_all (spell);
}

static void 
clear_session_cb (GtranslatorSpellChecker          *checker,
		  GtranslatorAutomaticSpellChecker *spell)
{
	gtranslator_automatic_spell_checker_recheck_all (spell);
}
 ...
 */
static gboolean
popup_menu_event (GtkTextView *view, GtranslatorAutomaticSpellChecker *spell) 
{
	GtkTextIter iter;
...
GtranslatorAutomaticSpellChecker *
gtranslator_automatic_spell_checker_new (GtkTextBuffer     *doc,
				   GtranslatorTab       *tab,
				   GtranslatorSpellChecker *checker)
{
	GtranslatorAutomaticSpellChecker *spell;

	GtkTextIter start, end;

	g_return_val_if_fail (GTRANSLATOR_IS_SPELL_CHECKER (checker), NULL);
	g_return_val_if_fail ((spell = gtranslator_automatic_spell_checker_get_from_tab (tab)) == NULL,
			      spell);
	
	/* attach to the widget */
	spell = g_new0 (GtranslatorAutomaticSpellChecker, 1);
	...
	if (automatic_spell_checker_id == 0)
		automatic_spell_checker_id = 
			g_quark_from_static_string ("GtranslatorAutomaticSpellCheckerID");
	if (suggestion_id == 0)
		suggestion_id = 
			g_quark_from_static_string ("GtranslatorAutoSuggestionID");
	...
	spell->mark_insert_start = gtk_text_buffer_get_mark (GTK_TEXT_BUFFER (doc),
					"gtranslator-automatic-spell-checker-insert-start");
	...
	spell->mark_insert_end = gtk_text_buffer_get_mark (GTK_TEXT_BUFFER (doc),
					"gtranslator-automatic-spell-checker-insert-end");
	...
	spell->mark_click = gtk_text_buffer_get_mark (GTK_TEXT_BUFFER (doc),
					"gtranslator-automatic-spell-checker-click");
...
GtranslatorAutomaticSpellChecker *
gtranslator_automatic_spell_checker_get_from_tab (const GtranslatorTab *tab)
{

	if (automatic_spell_checker_id == 0)
		return NULL;

	return g_object_get_qdata (G_OBJECT (tab), automatic_spell_checker_id);
}	

void
gtranslator_automatic_spell_checker_free (GtranslatorAutomaticSpellChecker *spell)
{
	g_return_if_fail (spell != NULL);
	g_return_if_fail (gtranslator_automatic_spell_checker_get_from_tab (spell->tab) == spell);
	
	if (automatic_spell_checker_id == 0)
		return;

	g_object_set_qdata (G_OBJECT (spell->tab), automatic_spell_checker_id, NULL);	
}

static void
gtranslator_automatic_spell_checker_free_internal (GtranslatorAutomaticSpellChecker *spell)
{
	...
	while (list != NULL)
	{
		GtranslatorView *view = list->data;
...
void
gtranslator_automatic_spell_checker_attach_view (
		GtranslatorAutomaticSpellChecker *spell, 
...
void 
gtranslator_automatic_spell_checker_detach_view (
		GtranslatorAutomaticSpellChecker *spell,
		GtranslatorView *view)
{
	g_return_if_fail (spell != NULL);
